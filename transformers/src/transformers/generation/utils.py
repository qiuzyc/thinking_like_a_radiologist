    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            next_token_scores = logits_processor(input_ids[:batch_size], next_token_logits)
            if do_sample:
                next_token_scores = logits_warper(input_ids[:batch_size], next_token_scores)
            ...
            # make sure that every conditionings next tokens are the same.
            conditionings_cnt = int(input_ids.shape[0] / next_tokens.shape[0])
            next_tokens = next_tokens.repeat(conditionings_cnt)
